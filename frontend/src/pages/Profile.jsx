...
    const navigate = useNavigate() // navigate hook joda gaya hai
    const [open, setOpen] = useState(false)
    const [loading, setLoading] = useState(false)
    const { user } = useSelector(store => store.auth)

    // FIX: Initial state mein null checks (agar user abhi load nahi hua ho)
    const [input, setInput] = useState({
        firstName: user?.firstName || '',
        lastName: user?.lastName || '',
        occupation: user?.occupation || '', // Occupation default value
        bio: user?.bio || '',
...
    })
    ...
    // FIX: Agar user logged out hai to handle karein
    if (!user) {
        navigate('/login');
        return null; 
    }
...
        formData.append("occupation", input.occupation); // Occupation field append kiya
...
                                    </div>
                                ...
                                {/* ADDED OCCUPATION INPUT FIELD HERE */}
                                <div className='flex gap-2'>
                                ...
                                {/* END of ADDED OCCUPATION INPUT FIELD */}
